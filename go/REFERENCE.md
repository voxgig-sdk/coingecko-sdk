# Coingecko Golang SDK Reference

Complete API reference for the Coingecko Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *CoingeckoSDK`
...
#### `General(data map[string]any) CoingeckoEntity`

Create a new `General` entity instance. Pass `nil` for no initial data.

#### `Simple(data map[string]any) CoingeckoEntity`
...
```go
result, err := client.Simple(nil).Load(map[string]any{"ids": "ids", "vs_currency": "vs_currency"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
