...
import (
	...
	omni "github.com/voxgig-sdk/coingecko-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CoingeckoError); is {
				out := map[string]any{
					"name":    "CoingeckoError",
...
func omniCtx(arg any, client *sdk.CoingeckoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CoingeckoSDK); is {
			client = live
