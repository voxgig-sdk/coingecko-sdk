...
func TestSimpleEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		simpleRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.simple")))
...
func simpleBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COINGECKO_TEST_SIMPLE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COINGECKO_TEST_SIMPLE_ENTID"])
