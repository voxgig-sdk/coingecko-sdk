...
class CoingeckoSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CoingeckoHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CoingeckoHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CoingeckoFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CoingeckoFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CoingeckoUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CoingeckoHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CoingeckoHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CoingeckoHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CoingeckoError($op . "_allow",
                "CoingeckoSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CoingeckoHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CoingeckoError("graphql_error",
                "CoingeckoSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Simple($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_simple === null) {
                $this->_simple = new SimpleEntity($this, null);
            }
            return $this->_simple;
        }
        return new SimpleEntity($this, $data);
    }
