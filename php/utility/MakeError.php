...
// Coingecko SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class CoingeckoMakeError
{
    public static function call(?CoingeckoContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CoingeckoContext([], null);
        }
        $op = $ctx->op ?? new CoingeckoOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CoingeckoResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CoingeckoError) ? $err->msg : (string)$err;
        $msg = "CoingeckoSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new CoingeckoError('', $msg, $ctx);
