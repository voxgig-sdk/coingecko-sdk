...
class CoingeckoNetsimFeature extends CoingeckoBaseFeature
{
    ...
    private function _simulate(CoingeckoContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(CoingeckoContext $ctx, array $applied): void
    {
        $client = $this->client;
