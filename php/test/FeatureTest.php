...
class FtCtrl extends CoingeckoControl
{
...
class FtClient
{
    ...
    private CoingeckoUtility $utility;

    public function __construct(CoingeckoUtility $utility, array $options)
    {
    ...
    public function get_utility(): CoingeckoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CoingeckoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CoingeckoContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CoingeckoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CoingeckoConfig::make_feature($name);
    ...
    private static function buildUrl(CoingeckoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CoingeckoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CoingeckoResponse($fetched);
        }
    ...
    private function fail(CoingeckoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CoingeckoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
