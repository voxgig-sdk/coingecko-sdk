...
class SimpleEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new SimpleEntity($this->_client, $opts);
    }

    /**
     * @param Simple|array $args Simple data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CoingeckoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Simple|array The current Simple data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Simple fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CoingeckoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Simple fields).
     */
    ...
    /**
     * Load a single Simple.
     *
     * @param SimpleLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed SimpleLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Simple|array The loaded Simple as an assoc-array at the
     *   SDK boundary; throws CoingeckoError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CoingeckoHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
