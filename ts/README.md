...
The TypeScript SDK for the Coingecko API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.General()` — each with a small set of operations (`load`)
...
```ts
const client = new CoingeckoSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `CoingeckoSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `CoingeckoSDK.test(testopts?, sdkopts?)` | `CoingeckoSDK` | Create a test-mode client. |
...
```ts
const simple = await client.Simple().load({ ids: 'ids', vs_currency: 'vs_currency' })
```
...
coingecko/
├── src/
│   ├── CoingeckoSDK.ts        # Main SDK class
