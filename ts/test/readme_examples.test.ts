...
//     (\`new CoingeckoSDK(...)\` / \`CoingeckoSDK.test(...)\` become
//     \`CoingeckoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"general":{"test01":{"id":"test01"}},"simple":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CoingeckoSDK(...)\` and any
// \`CoingeckoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CoingeckoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CoingeckoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
