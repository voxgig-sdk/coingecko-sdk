...
module CoingeckoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CoingeckoResponse.new({})
    ...
    if fetch_err
      ...
      response = CoingeckoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CoingeckoResponse.new(fetched)
    else
