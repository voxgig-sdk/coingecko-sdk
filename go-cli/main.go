// Command Coingecko-cli is a CLI + REPL for the Coingecko SDK,
...
const prompt = "coingecko"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "general simple"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      COINGECKO_APIKEY (api key), COINGECKO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: COINGECKO_APIKEY carries the API key and
	// COINGECKO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("COINGECKO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("COINGECKO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCoingeckoSDK(opts)
