...
boru-driven command-line client **and** interactive REPL for the Coingecko
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/coingecko-cli)
make build
...
./coingecko-cli load 1 general            # {id:1} shorthand
./coingecko-cli load '{id:1}' general       # explicit match map

# 5. Override the API base URL for a single call
COINGECKO_BASE=https://api.example.com ./coingecko-cli load 1 general

# 6. No arguments -> interactive REPL
./coingecko-cli
coingecko> load 1 general
coingecko> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/coingecko-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/coingecko-cli load 1 general
   ```
...
```sh
./coingecko-cli load 1 general          # scalar shorthand for {id:1}
./coingecko-cli load '{id:1}' general     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `coingecko>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `general`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/coingecko-cli`. |
...
(`./coingecko-cli <expr>`) and interactively (the REPL), and expressions compose the
