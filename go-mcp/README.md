...
[MCP](https://modelcontextprotocol.io) server exposing the Coingecko SDK as
two agent tools — `coingecko_list` and `coingecko_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/coingecko-mcp)
make build

# 2. Provide credentials via the environment
export COINGECKO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user coingecko \
  -- /absolute/path/to/coingecko-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./coingecko-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// coingecko_list: first page of records
{ "entity": "general" }
{ "entity": "general", "query": { } }

// coingecko_load: one record by id
{ "entity": "general", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/coingecko-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user coingecko \
     -- "$PWD"/dist/*/coingecko-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `coingecko_list` and `coingecko_load` tools now appear
   in new sessions. Ask the agent to *"list general using coingecko"*
   and it calls `coingecko_list` with `{"entity":"general"}`.
...
```sh
export COINGECKO_APIKEY=sk_live_xxx            # API key
export COINGECKO_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `coingecko_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `coingecko_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `COINGECKO_APIKEY` | API key sent with every request. |
| `COINGECKO_BASE` | Optional override of the API base URL. |
...
```sh
./coingecko-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"coingecko_load","arguments":{"entity":"general","query":{"id":1}}}}'
```
