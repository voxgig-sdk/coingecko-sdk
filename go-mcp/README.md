...
MCP server exposing the Coingecko SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `coingecko_list` | `entity`, optional `query` map | First page of records as JSON |
| `coingecko_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `coingecko_list` and
`coingecko_load` tools become available in new sessions.
