# Coingecko SDK test runner

from __future__ import annotations
import os
import json

from coingecko_sdk.utility.voxgig_struct import voxgig_struct as vs


class CoingeckoTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CoingeckoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CoingeckoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CoingeckoTestRunner.getenv("COINGECKO_TEST_LIVE")
        override = CoingeckoTestRunner.getenv("COINGECKO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CoingeckoTestRunner.getenv(key)
        ...
        explain = CoingeckoTestRunner.getenv("COINGECKO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["COINGECKO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CoingeckoTestRunner._test_control is not None:
            return CoingeckoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CoingeckoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CoingeckoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CoingeckoTestRunner.load_test_control()
...
def load_env_local():
    CoingeckoTestRunner.load_env_local()


def env_override(m):
    return CoingeckoTestRunner.env_override(m)


def entity_data(v):
    return CoingeckoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CoingeckoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CoingeckoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CoingeckoTestRunner.load_test_control()


def live_client_options():
    return CoingeckoTestRunner.live_client_options()


def live_delay_ms():
    return CoingeckoTestRunner.live_delay_ms()
