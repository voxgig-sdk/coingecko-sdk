...
The Python SDK for the Coingecko API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.General(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.Coingecko(None).load(
    {"id": "test01"}, None
)
