...
local CoingeckoSDK = {}
CoingeckoSDK.__index = CoingeckoSDK
...
CoingeckoSDK._make_feature = _make_feature


function CoingeckoSDK.new(options)
  local self = setmetatable({}, CoingeckoSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function CoingeckoSDK:options_map()
...
function CoingeckoSDK:get_utility()
  return Utility.copy(self._utility)
end


function CoingeckoSDK:get_root_ctx()
  return self._rootctx
end


function CoingeckoSDK:prepare(fetchargs)
...
function CoingeckoSDK:direct(fetchargs)
...
-- by passing it.
function CoingeckoSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function CoingeckoSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "CoingeckoSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function CoingeckoSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = CoingeckoSDK.new(sdkopts)
  sdk.mode = "test"
