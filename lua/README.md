...
The Lua SDK for the Coingecko API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:General(nil):load({ id = "example_id" }, nil)
...
local result, err = client:Coingecko(nil):load(
  { id = "test01" }, nil
)
