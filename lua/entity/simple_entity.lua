...
function SimpleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SimpleEntity)
  self._name = "simple"
  self._client = client
...
function SimpleEntity:get_name()
  return self._name
end


function SimpleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SimpleEntity.new(self._client, opts)
end
...
function SimpleEntity:mark_deleted()
  self._deleted = true
end


function SimpleEntity:deleted()
  return true == self._deleted
end


function SimpleEntity:data_set(args)
  if args ~= nil then
...
function SimpleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SimpleEntity:match_set(args)
  if args ~= nil then
...
function SimpleEntity:match_get()
...
function SimpleEntity:stream(action, args, callopts)
...
---@param reqmatch SimpleLoadMatch
---@param ctrl? table
---@return Simple
---@return string? err
function SimpleEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function SimpleEntity:_run_op(ctx, post_done)
  local utility = self._utility
