...
class SimpleEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = simple_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "simple." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set COINGECKO_TEST_SIMPLE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    simple_ref01_data = nil
    if simple_ref01_data_raw.length > 0
...
def simple_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "simple", "SimpleTestData.json")
  ...
  entid_env_raw = ENV["COINGECKO_TEST_SIMPLE_ENTID"]
  ...
  if env["COINGECKO_TEST_LIVE"] == "TRUE"
    ...
    client = CoingeckoSDK.new(Helpers.to_map(merged_opts))
  end
