...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = CoingeckoOmni.make_runner(TEST_JSON_FILE, CoingeckoSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : CoingeckoOmni.errify(err)
    end
