...
class SimpleEntity
  ...
  def make
    opts = @_entopts.dup
    SimpleEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CoingeckoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Simple, Hash] the current Simple data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CoingeckoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Simple fields)
  def match_get
  ...
  #
  # @param reqmatch [SimpleLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Simple.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Simple, Hash] the loaded Simple; raises CoingeckoError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CoingeckoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
