...
require_relative 'Coingecko_types'
...
class CoingeckoSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CoingeckoHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CoingeckoHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CoingeckoFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CoingeckoUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CoingeckoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CoingeckoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CoingeckoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = CoingeckoHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CoingeckoError.new(
        "graphql_error", "CoingeckoSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
