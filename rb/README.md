...
The Ruby SDK for the Coingecko API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.General` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = CoingeckoSDK.test(testopts, sdkopts)
```
...
raise a `CoingeckoError` (a `StandardError` subclass), so wrap
...
simple = client.Simple.load({ "ids" => "ids", "vs_currency" => "vs_currency" })
```
...
The main module (`Coingecko_sdk`) exports the SDK class
